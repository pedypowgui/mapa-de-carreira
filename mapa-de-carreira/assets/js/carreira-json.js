...
const renderProfile = ({ profile, contacts }) => {
	...
	contacts.forEach((contact) => {
		...
		const link = createElement("a", `bi ${contact.icon} text-link`, contact.label);
...
const renderCareerTimeline = (careerSteps) => {
	...
	careerSteps.forEach((step, index) => {
		const article = createElement("article", "resume-timeline-item position-relative pb-4");
		...
		description.children[1].id = `soft-passo${index + 1}`;
		...
		description.children[4].id = `roadmap${index + 1}`;
		description.children[3].id = `hard-passo${index + 1}`;
